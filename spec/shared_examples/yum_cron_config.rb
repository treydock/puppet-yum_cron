shared_examples 'yum_cron::config' do |facts|
  ...
  when '7'
    EL7_CONFIGS.each do |config|
      it "should set #{config[:name]} to #{config[:value]}" do
        should contain_yum_cron_config(config[:name]).with({
          :value  => config[:value],
    ...
    it { should have_yum_cron_config_resource_count(EL7_CONFIGS.size) }
    it { should have_shellvar_resource_count(0) }

    context 'when enable => false' do
      let(:params) {{ :enable => false }}
      EL7_CONFIGS.each do |config|
        it "should not notify Service[yum-cron]" do
          should contain_yum_cron_config(config[:name]).without_notify
        end
      end
    end
    ...
    context 'extra_configs defined' do
      ...
      it do
        should contain_yum_cron_config('email/email_from').with({
          :value  => 'foo@bar.com',
          :notify => 'Service[yum-cron]',
        })
      end
  ...
  when '6'
    EL6_CONFIGS.each do |config|
      it "should set #{config[:name]} to #{config[:value]}" do
        should contain_shellvar("yum_cron #{config[:name]}").with({
          :ensure   => 'present',
    ...
    it { should have_yum_cron_config_resource_count(0) }
    it { should have_shellvar_resource_count(EL6_CONFIGS.size) }

    context 'when enable => false' do
      let(:params) {{ :enable => false }}
      EL6_CONFIGS.each do |config|
        it "should not notify Service[yum-cron]" do
          should contain_shellvar("yum_cron #{config[:name]}").without_notify
        end
      end
    end
    ...
    context 'extra_configs defined' do
      ...
      it do
        should contain_shellvar('yum_cron ERROR_LEVEL').with({
          :ensure   => 'present',
          :target   => '/etc/sysconfig/yum-cron',
          :notify   => 'Service[yum-cron]',
  ...
  when '5'
    ...
    it { should have_yum_cron_config_resource_count(0) }
    it { should have_shellvar_resource_count(EL5_CONFIGS.size) }
